...
public class EmailMessageHandler(ProcessedMessageStore processedMessageStore, ILogger<EmailMessageHandler> logger)
{
    public async Task<MessageOutcome> HandleAsync(byte[] body, CancellationToken stoppingToken)
    {
        ...
        // message does not produce a duplicate email
        ...
        logger.LogInformation("Message {Id} - Sending email to {Email}: Dear {FullName}, {Body}",
            message.Id, message.Email, message.FullName, message.Body);
