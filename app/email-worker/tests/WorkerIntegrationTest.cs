...
public class WorkerIntegrationTest(RabbitMqFixture fixture, ITestOutputHelper testOutputHelper) : IClassFixture<RabbitMqFixture>
{
    ...
    public async Task OnReceived_WhenConcurrentIdenticalValidMessage_OnlyProcessesOnce()
    {
        ...
        // Deliver the same message concurrently five times to simulate concurrent identical deliveries
        var publishTasks = Enumerable.Range(0, 5)
            .Select(_ => pubChannel.BasicPublishAsync(exchange: EmailConsumer.ExchangeName, routingKey: "", body: body))
            .ToArray();
        ...
        // Wait until all five concurrent deliveries have been handled: the first
        // one sends the email, the remaining four are deduplicated
        ...
        //Assert that only one email was sent and the rest were skipped
        fixture.FakeLogger.Collector.GetSnapshot()
            .Count(r => r.Message.Contains(message.Id) && r.Message.Contains($"Sending email to {message.Email}"))
            .Should().Be(1, "a previously processed message must not be sent again");
        fixture.FakeLogger.Collector.GetSnapshot()
